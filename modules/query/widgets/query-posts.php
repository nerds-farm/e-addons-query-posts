...
class Query_Posts extends Base_Query {
    ...
    public function get_query_args() {        
        ...
        */
        ...
        if( !empty($settings['exclude_myself']) ) array_push( $excludedPosts, get_the_ID() );
        $args['post__not_in'] = $excludedPosts;
        // ignore_sticky_posts
        if( !empty($settings['ignore_sticky_posts']) && $settings['ignore_sticky_posts'] ){
            $args['post__in'] = get_option( 'sticky_posts' );
            $args['ignore_sticky_posts'] = $settings['ignore_sticky_posts'] == 'yes' ? 1 : false;
        }
        /*
        '1 - automatic_mode'
        '2 - get_cpt'
